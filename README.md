...
- **Arquitetura modular e sem dependências.** Cada responsabilidade vive em seu próprio módulo encapsulado (`parser`, `mask`, `translator`, `lookup`, `app`), no padrão IIFE com namespace, expondo apenas uma API pública. Nenhum framework, nenhuma biblioteca externa — JavaScript puro.
